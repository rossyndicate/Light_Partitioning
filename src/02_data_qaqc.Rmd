...
nap_resid <- nap_mods %>%
  dplyr::select(-mods) %>%
  unnest(data) %>%
  mutate(ratio_rmse = paste('ratio =',chl_ratio,', RMSE =', round(rmse(kd,pred),3)))
...
- ADD SQUARE METHOD for chl-a to biomass

- CV of RMSE, 

- Keep most abundant methods? 
